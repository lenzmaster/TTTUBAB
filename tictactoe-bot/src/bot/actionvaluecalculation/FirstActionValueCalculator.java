...
public class FirstActionValueCalculator implements IActionValueCalculator{
	...
	public float calculate(MCSTNode node, MCSTNode lastUpdatedChildNode) {
		...
		} else if (lastUpdatedChildNode.getEvaluationValue() == GlobalDefinitions.NODE_EVALUATION_UPPER_BOUND
				|| lastUpdatedChildNode.getEvaluationValue() == GlobalDefinitions.NODE_EVALUATION_LOWER_BOUND) {//child was wining node --> only care about winning child, when calculating the action value
