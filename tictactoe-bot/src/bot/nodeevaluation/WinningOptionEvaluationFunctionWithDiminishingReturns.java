...
/**
 * Usage of diminishing returns for additonal winning options.
 * @author André
 *
 */
public class WinningOptionEvaluationFunctionWithDiminishingReturns extends ConicalCombinationFunctionAbstract{

	private static int potencyBasis = 3;
	
	private static float diminishingFactor = 0.4f;
	...
	private static Logger LOGGER = new Logger("WinningOptionEvaluationFunctionWithDiminishingReturns");
	...
	private float[] preCalculatedDiminishingFactors = null;
	...
	private float getDenominator(){
		if (denominator == -1){
			denominator = ((float) Math.pow(potencyBasis, 4));
		}
		return denominator;
	}
	
	private float[] getPreCalculatedDiminishingFactors(){
		if (preCalculatedDiminishingFactors == null){
			initalizeExponentiatedDimishingFactors();
		}
		return preCalculatedDiminishingFactors;
	}
	
	/**
	 * Constructor
	 */
	public WinningOptionEvaluationFunctionWithDiminishingReturns(){
		initalizeExponentiatedDimishingFactors();
	}

	private void initalizeExponentiatedDimishingFactors(){	
	...
	private float calcBoardScore(float[] winningOptionsScores){
		...
		float[] weightedFactors = getPreCalculatedDiminishingFactors();
		int counter = 0;
		int indexOflastElementOfAscOrderedScores = ascOrderedScores.length-1;
		while (counter < weightedFactors.length && counter < ascOrderedScores.length){
			//asc ordered factors backwards, due to ascending order
			weightedWinningOptionScoreSum += weightedFactors[counter] * ascOrderedScores[indexOflastElementOfAscOrderedScores - counter];
			counter++;
		}
	...
	public float calculate() {
		...
		if (normedScoreDifference > 0.5 || normedScoreDifference < -0.5){
			LOGGER.log("Absolute normed score difference is above 0.5: " + normedScoreDifference);
